...
New-dynasty initialization derives an isolated seeded RNG from the universe version, roster-generation version, typed dynasty seed, and Program ID. It generates each roster through the accepted Team generator, assigns the stable Program ID to Team, and derives a legal default Rotation. The full Program dataset remains canonical in `src/universe`; the six-program exhibition catalog is still only a UI subset plus one development fixture. No schedule or season exists yet.
