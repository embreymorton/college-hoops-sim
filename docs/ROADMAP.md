...
- **6E.16A — Recruiting Page Density + Guidance Polish:** NEXT — Claude Code (`frontend-design`), presentation-only. Tighten page-level vertical rhythm; consolidate Board/Signed/Openings/Offers/Needs into a compact overview; remove duplicate or overweight status presentation; establish one coherent Recruiting guide/help destination; and make zero-openings Late Recruiting copy acknowledge a complete class. Preserve Board/Battles/National responsibilities and all Recruiting mechanics.
- **6E.16B — Season Hub + League Information Hierarchy Polish:** QUEUED — Claude Code (`frontend-design`), presentation-only. Tighten Hub rhythm; distinguish unresolved Focus Targets from controlled-Program Commits; integrate Recruiting Update into the Recruiting module without changing its session semantics; stabilize and densify the Quick Sim game-card states; retain only controlled Conference standings on the Hub and compose them with Recent Results where appropriate; reduce redundant root League chrome while preserving Team/Player detail Back behavior. No League or domain changes.

After this deliberate two-milestone UI checkpoint, the next evidence-backed QOL
candidates are Coaching/roster navigation and Followed Players. League News, offseason
