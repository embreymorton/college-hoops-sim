...
- Added a distinct Tournament context to active/former Player careers with
  career production, achievements, season runs, game history, career highs,
  and existing MOP context.
- Added a separate Tournament Record Book for PTS/REB/AST/STL/BLK across Single
  Game, Tournament Run totals, and Career totals.
- Derived all new history from active/archived Postseason facts and stable
  Player IDs without changing regular-season semantics or adding canonical
  historical state, then returned to an Open Planning Checkpoint.
