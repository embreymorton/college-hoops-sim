...
- **6E.16B — Season Hub + League Information Hierarchy Polish:** NEXT — Claude Code (`frontend-design`), presentation-only. Tighten Hub rhythm; distinguish unresolved Focus Targets from controlled-Program Commits; integrate Recruiting Update into the Recruiting module without changing its session semantics; stabilize and densify the Quick Sim game-card states; retain only controlled Conference standings on the Hub and compose them with Recent Results where appropriate; reduce redundant root League chrome while preserving Team/Player detail Back behavior. No League or domain changes.
