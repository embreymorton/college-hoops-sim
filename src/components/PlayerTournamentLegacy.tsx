...
export function PlayerTournamentLegacy({
  ...
  return <section className="section tournament-legacy" aria-labelledby="tournament-legacy-heading">
    <div className="section-heading"><h2 id="tournament-legacy-heading" className="section-title">Tournament Legacy</h2><p className="eyebrow-tag">Tournament Only</p></div>
    <div className="player-stat-block player-stat-block--legacy">
      ...
      <div className="stat-trio player-stat-block__row player-stat-block__row--secondary">
        <div className="stat-trio__item"><span className="stat-trio__value">{career.tournamentAppearances}</span><span className="stat-trio__label">Apps</span></div>
        ...
        <div className="stat-trio__item"><span className="stat-trio__value">{career.nationalChampionships}</span><span className="stat-trio__label">Titles</span></div>
    ...
    <div className="table-scroll"><table className="data-table"><thead><tr><th>Season</th><th>Program</th><th>Finish</th><th>GP</th><th>PTS</th><th>REB</th><th>AST</th></tr></thead>
      <tbody>{career.runs.slice().reverse().map((run) => <tr key={run.seasonNumber}>
        ...
        <td>{finishLabel(run.finish)}{run.isMop ? ' · MOP' : ''}</td><td>{run.stats.gamesPlayed}</td><td>{run.stats.points}</td><td>{run.stats.rebounds}</td><td>{run.stats.assists}</td>
      </tr>)}</tbody></table></div>
    ...
    <h3 className="section-subtitle">Tournament Game History</h3>
    <div className="table-scroll"><table className="data-table game-log-table"><thead><tr><th>Season</th><th>Round</th><th>Opponent</th><th>Result</th><th>Min</th><th>Pts</th><th>Reb</th><th>Ast</th></tr></thead>
