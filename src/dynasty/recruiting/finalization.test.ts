...
describe('Late Recruiting and finalization', () => {
  ...
  it('keeps season-long relationship leadership authoritative in the late market', () => {
    ...
    const recruiting: RecruitingState = {
      ...
      programs: {
        [programA!]: { programId: programA!, projectedOpeningsByPosition: openings, board: [{ playerId: recruit.player.id, origin: 'assistant' as const, isFocused: true, hasActiveOffer: true }] },
        [programB!]: { programId: programB!, projectedOpeningsByPosition: openings, board: [{ playerId: recruit.player.id, origin: 'assistant' as const, isFocused: true, hasActiveOffer: true }] },
      },
      relationshipProgressByPlayerId: {
    ...
    const recruiting: RecruitingState = {
      ...
      programs: {
        ...
        [programB!]: {
          programId: programB!,
          projectedOpeningsByPosition: openings,
          board: [{ playerId: premium.player.id, origin: 'assistant' as const, isFocused: true, hasActiveOffer: true }],
        },
      },
      relationshipProgressByPlayerId: {
    ...
    const recruiting: RecruitingState = {
      ...
      programs: {
        [programId]: {
          programId,
          projectedOpeningsByPosition: openings,
          board: [{ playerId: lower.player.id, origin: 'assistant' as const, isFocused: true, hasActiveOffer: true }],
        },
      },
      relationshipProgressByPlayerId: {
